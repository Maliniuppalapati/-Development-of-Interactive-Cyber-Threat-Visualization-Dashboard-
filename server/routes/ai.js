...
// Protected Email Analysis Endpoint
router.post('/analyze-email', protect, requireRole('user'), async (req, res) => {
    try {
        const response = await axios.post(`${AI_ENGINE_URL}/analyze-email`, req.body, {
            timeout: 35000
        });
        res.json(response.data);
    } catch (error) {
        console.error('AI Email Analysis Error:', error.message);
