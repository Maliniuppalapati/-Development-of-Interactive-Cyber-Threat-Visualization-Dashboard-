...
const ThreatFeed = () => {
  ...
  const handleExportCSV = () => {
    if (filteredThreats.length === 0) {
      toast({ title: 'No Data', description: 'No threats to export.', variant: 'destructive' });
      return;
    }

    const headers = ['ID', 'Type', 'Severity', 'Source IP', 'Source Country', 'Target IP', 'Target Country', 'MITRE Tactic', 'MITRE ID', 'Status', 'Timestamp'];
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `threat_export_${new Date().getTime()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    toast({ title: 'Export Successful', description: `${filteredThreats.length} threats exported to CSV.` });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" onClick={handleExportCSV} disabled={filteredThreats.length === 0}>
