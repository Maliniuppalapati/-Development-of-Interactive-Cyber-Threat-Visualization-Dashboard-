...
const Incidents = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedIncident = { 
            ...
            timeline: updated.timeline.map((t: any) => ({ ...t, timestamp: new Date(t.timestamp) })) 
        };

        setIncidents(prev => {
            const exists = prev.find(i => i.id === updated.id);
            if (exists) {
                return prev.map(i => i.id === updated.id ? updatedIncident : i);
            } else {
                // If it scrolled out of view, put it back at the top
  ...
  const filteredIncidents = incidents.filter(
    (inc) => {
      ...
      const matchesFilter = statusFilter ? inc.status.toLowerCase() === statusFilter.toLowerCase() : true;
  ...
  const handleExportCSV = () => {
    if (filteredIncidents.length === 0) {
      toast({ title: 'No Data', description: 'No incidents to export.', variant: 'destructive' });
      return;
    }

    const headers = ['ID', 'Title', 'Severity', 'Status', 'Assignee', 'Created', 'Affected Assets'];
    const csvContent = [
      headers.join(','),
      ...filteredIncidents.map(inc => [
        ...
        inc.status,
        inc.assignee,
        inc.created.toISOString(),
        inc.affectedAssets.join('; ')
      ].map(v => `"${v}"`).join(','))
    ].join('\n');
    ...
    link.setAttribute('download', `incidents_export_${new Date().getTime()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    toast({ title: 'Export Successful', description: `${filteredIncidents.length} incidents exported to CSV.` });
  };
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card 
            className={cn("soc-card cursor-pointer transition-all hover:border-primary/50", statusFilter === null && "ring-2 ring-primary")}
          ...
          <Card 
            className={cn("soc-card cursor-pointer transition-all hover:border-info/50", statusFilter === 'open' && "ring-2 ring-info")}
          ...
          <Card 
            className={cn("soc-card cursor-pointer transition-all hover:border-warning/50", statusFilter === 'investigating' && "ring-2 ring-warning")}
            onClick={() => setStatusFilter(statusFilter === 'investigating' ? null : 'investigating')}
          >
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
          ...
          <Card 
            className={cn("soc-card cursor-pointer transition-all hover:border-accent/50", statusFilter === 'resolved' && "ring-2 ring-accent")}
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" onClick={handleExportCSV} disabled={filteredIncidents.length === 0}>
